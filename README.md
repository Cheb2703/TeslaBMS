...
Each time the charger switches on, the firmware reads its charge settings (current, voltage, float, taper) back and checks they match what was sent. A check that fails is repeated a few seconds later, and if it still fails three times in a row the charger is switched off and stays off until you switch it on again, so it can never quietly charge to the wrong limits. Set `CHARGER_VERIFY_TURNS_OFF` to `0` in `src/config.h` if you would rather it only wrote a message to the log.
